...
let RADIUS_LAYER=null; // (KHÔNG dùng nữa để vẽ ở tâm map; giữ biến chỉ để tương thích)
...
/* ========= PapaParse loader (đảm bảo luôn có) ========= */
async function ensurePapa() {
  ...
  await new Promise((resolve, reject) => {
    ...
    s.defer = true;
    s.onload = resolve;
    s.onerror = () => reject(new Error('Không tải được PapaParse từ CDN'));
...
async function loadCSV(){
  try{
    ...
    const resp=await fetch(url,{cache:'no-store', mode:'cors', signal:sig});
    if(!resp.ok){ const msg=`Không tải được CSV (${resp.status})`; console.error(msg,{url,st:resp.status}); $status && ($status.textContent=msg); alert(msg); return; }

    const text=await resp.text(); if(sig.aborted) return;
    if(!text||!/[,;\n]/.test(text)){ const msg='CSV rỗng/không hợp lệ.'; console.error(msg); $status && ($status.textContent=msg); alert(msg); return; }

    const parsed = Papa.parse(text,{header:true,skipEmptyLines:true,dynamicTyping:false});
    if (parsed.errors?.length) console.warn('Papa errors:', parsed.errors.slice(0,3));
    RAW = Array.isArray(parsed.data) ? parsed.data : [];
    if(!RAW.length){ const msg='Không có dữ liệu'; console.error(msg); $status && ($status.textContent=msg); alert(msg); return; }

    const s=RAW[0], keys=Object.keys(s), find=(alts,fb)=>keys.find(k=>alts.some(a=>new RegExp(`^${a}$`,'i').test(k.trim())))
                                               || keys.find(k=>alts.some(a=>k.toLowerCase().includes(a))) || fb;
    KEY.lat= find(['lat','latitude','vido','vi_do','vĩ độ','y','vi do'],'lat');
    KEY.lng= find(['lng','lon','longitude','kinhdo','kinh_do','kinh độ','x','kinh do'],'lng');
    KEY.ten= find(['ten','tên','name','khach'],'ten');
    KEY.ma = find(['ma_kh','makh','ma','mã'],'ma_kh');
    KEY.sdt= find(['sdt','so_dt','so_dien_thoai','dien_thoai','phone'],'sdt');
    KEY.dc = find(['dia_chi','địa chỉ','dia chi','address','addr'],'dia_chi');
    KEY.nv = find(['nv','nv_giao','nv_giao_hang','nhan_vien','nhanvien'],null);
    KEY.status=find(['trang_thai','trangthai','status','tinh_trang'],null);

    renderFiltered();
    $status && ($status.textContent=`Đã tải ${RAW.length} dòng.`);
  }catch(e){
    console.error('loadCSV failed:', e);
    const msg='Lỗi khi tải CSV: '+(e.message||e);
...
/* ========= LỌC THEO BÁN KÍNH VỊ TRÍ CỦA TÔI ========= */
function okByMyRadius(lat,lng){
  if (!mapCfg.showRadius || !MY_MARKER) return true;
  const center = MY_MARKER.getLatLng();
  const maxM   = (mapCfg.radiusKm||0)*1000;
  return distM(center,{lat,lng}) <= maxM;
}

/* ========= RENDER/FILTER ========= */
function renderFiltered(){
  GROUP.clearLayers(); ROUTES.clearLayers(); WARD_LABELS.clearLayers(); NEAR_LABELS.clearLayers(); MARKERS_DATA=[];
  // KHÔNG vẽ RADIUS_LAYER tím theo tâm map nữa
  ...
  const centerForR=map.getCenter(); const rLimitKm=(Q.rKm&&Q.rKm>0)?Q.rKm:null; // filter theo r: trong ô tìm kiếm (tâm map)
  const wardAgg=new Map();

  for(const r of RAW){
    const fixed=bestFix(r[KEY.lat],r[KEY.lng]); if(!fixed) continue;
    if(vnOnly && !inVN(fixed.lat,fixed.lng)) continue;

    // Lọc theo radius của "tôi" (nếu bật showRadius)
    if (!okByMyRadius(fixed.lat, fixed.lng)) continue;
    ...
    const nv=KEY.nv?(r[KEY.nv]||'').toString():''; const st=KEY.status?(r[KEY.status]||'').toString():'';

    const text=(s)=>s.toString().toLowerCase();
    const matchGroup=(arr,val)=>!arr.length||arr.some(k=>text(val).includes(k));

    // filter theo r: trong ô tìm kiếm (so với tâm bản đồ)
    let okRadiusQ=true;
    if(rLimitKm){ const d=distM({lat:centerForR.lat,lng:centerForR.lng},{lat:fixed.lat,lng:fixed.lng})/1000; okRadiusQ=(d<=rLimitKm); }

    const ok = okRadiusQ && matchGroup(Q.ma,code) && matchGroup(Q.ten,name) && matchGroup(Q.sdt,phone)
  ...
  $status && ($status.textContent = count>0 ? `Đã hiển thị ${count} điểm.` : `Không có điểm phù hợp.`);
...
function drawRoutes(routes){
  ...
  if (mapCfg.showDistance){
    ...
    $status && ($status.textContent += ` • Tổng quãng đường: ${km} km`);
  }
}
...
function renderConfigPanel(){
  ...
  const preview=()=>{ 
    ...
    setBase(mapCfg.mapTheme); 
    rebuildGroup(); 
    // Cập nhật vòng bán kính theo "tôi"
    if (MY_MARKER){
      const pos = MY_MARKER.getLatLng();
      if (mapCfg.showRadius){
        if (!RADIUS_LAYER) {} // bỏ dùng RADIUS_LAYER tím
        // vẽ/điều chỉnh MY_RADIUS ngay trong drawMyLocation()
        drawMyLocation(pos.lat, pos.lng);
      }else{
        if (RADIUS_LAYER){ map.removeLayer(RADIUS_LAYER); RADIUS_LAYER=null; }
      }
    }
  ...
  ['cfg_autoFit','cfg_cluster','cfg_labelKH','cfg_tooltip','cfg_routeByNV','cfg_showDistance','cfg_colorByStatus','cfg_showRadius']
    .forEach(id=>document.getElementById(id).addEventListener('change',preview));
...
function drawMyLocation(lat,lng){
  ...
  // Vòng bán kính đỏ theo cấu hình
  const radiusM = (mapCfg.radiusKm||0)*1000;
  if(mapCfg.showRadius){
    if(!MY_RADIUS){ MY_RADIUS=L.circle(pos,{radius:radiusM,color:'#ef4444',weight:2,fillOpacity:.06}).addTo(map); }
    else { MY_RADIUS.setLatLng(pos); MY_RADIUS.setRadius(radiusM); }
  } else {
    if(MY_RADIUS){ map.removeLayer(MY_RADIUS); MY_RADIUS=null; }
  }
}
...
async function startMyLocation(ma_nv){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{ const {latitude:lat,longitude:lng,accuracy}=pos.coords; drawMyLocation(lat,lng); map.setView([lat,lng],Math.max(map.getZoom(),15)); ensureInView(lat,lng); saveMyLocation(ma_nv,lat,lng,accuracy); updateNearestNumbers(); updateProximityLabels(); renderFiltered();
  ...
  MY_WATCH=navigator.geolocation.watchPosition(pos=>{ const {latitude:lat,longitude:lng,accuracy}=pos.coords; drawMyLocation(lat,lng); ensureInView(lat,lng); saveMyLocation(ma_nv,lat,lng,accuracy); updateNearestNumbers(); updateProximityLabels(); renderFiltered();
...
function bindUI(){
  ...
  map.on('moveend',()=>{ 
    // KHÔNG vẽ RADIUS_LAYER tím theo tâm bản đồ nữa
    const qStr=document.getElementById('q')?.value||''; if(/\br:/.test(qStr)) renderFiltered();
    updateProximityLabels();
  });
}
