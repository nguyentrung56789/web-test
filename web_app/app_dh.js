...
// ĐÃ THÊM: chờ __CONFIG_READY để lấy Supabase URL/KEY từ /api/getConfig

(async function boot() {
  // 0) Chờ cấu hình sẵn sàng (nếu có)
  try {
    if (window.__CONFIG_READY?.then) {
      await window.__CONFIG_READY;
    }
  } catch (e) {
    alert('Thiếu cấu hình Supabase. Không thể khởi động ứng dụng.');
    return;
  }
  ...
  /* ===== Supabase (DB/Auth) ===== */
  // Lấy cấu hình đã được cod_config.js gán vào (COD_BASE/getConfig)
